...
namespace ChessNN
{
    ...
    public class NeuralNet : IDisposable
    {
        ...
        {
            //Adjust the board's orientation to the proper one for each player (black or white)
            Piece[,] values = Board.AdjustFlip(board.Pieces, isW);
            foreach (Piece p in values)
            {
                //If it's empty, don't score it
                if (p is Empty) { continue; }
                //If it's hostile, make its current value negative
                if (p.Player.IsW != isW) { p.CVal = -Math.Abs(p.CVal); }
                //If it's friendly, make its current value positive
                else { p.CVal = Math.Abs(p.CVal); }
            }
            //Foreach neuron, compute its values
