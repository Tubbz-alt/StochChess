            ...
            for (int i = 0; i <= foresight; i++)
            {
                try
                {
                    ...
                    for (int ii = 0; ii < Boards[i].Count; ii++)
                    {
                        ...
                        else { tempbs = Moves(Boards[i][ii], !isW); }
                            ...
                            if (!b2.Checks(isW))
                            {
                                ...
                                if (b2.WTurn != isW)
                                {
                                    ...
                                    if (bestBoards.Count < foresight)
                                    {
                                        for (int j = 0; j < bestBoards.Count; j++)
                                        {
                                            ...
                                            {
                                                bestBoards.Insert(j, b2); bestVals.Insert(j, Score(b2, isW)); break;
                                            }
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            for (int j = 0; j < bestBoards.Count; j++)
                                            {
                                                if (b2Score > bestVals[j]) { bestVals[j] = b2Score; bestBoards[j] = b2; break; }
                                                else
                                                {
                                                    if (j > 1) { bestVals[j - 1] = b2Score; bestBoards[j - 1] = b2; break; }
                                                    else { b2.Dispose(); break; }
                                                }
                                ...
                                else
                                {
                                    ...
                                    if (bestBoards.Count < foresight)
                                    {
                                        ...
                                        {
                                            double b2Score = Score(b2, isW);
                                            if (bestVals.Count >= j && b2Score < bestVals[j]) { bestBoards.Add(b2); bestVals.Add(Score(b2, isW)); break; }
                                            else
                                            {
                                                bestBoards.Insert(j, b2); bestVals.Insert(j, Score(b2, isW)); break;
                                            }
